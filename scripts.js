...
var JDstring =
    'rockstar rockstar ninja ninja rockstar team product engineer growth mindset product';
...
function wordFreq(JDstring) {
    var punct = /[\u2000-\u206F\u2E00-\u2E7F\u00B7\\0-9\\'!"#$%&()*+,\-.\/:;<=>?@\[\]^_`{|}~]/g;
    var space = /\s+/g;
    var wordArray = JDstring.replace(punct, '')
        .replace(space, ' ')
        .trim()
        .split(/\s+/);
...
function makeTable(freqMap) {
    ...
    for (var word in rankedKeys) {
        $('#tablebod').append(
            `<tr><td>${word}</td><td>${freqMap[word]}</td></tr>`
        );
    }
}